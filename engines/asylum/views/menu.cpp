...
void Menu::updateAudioOptions() {
	...
	do {
		...
		case 1: volumeValue = Config.musicVolume / 250 + 20; break;
		case 2: volumeValue = Config.ambientVolume / 250 + 20; break;
		case 3: volumeValue = Config.sfxVolume / 250 + 20; break;
		case 4: volumeValue = Config.voiceVolume / 250 + 20; break;
		case 5: volumeValue = Config.movieVolume / 250 + 20; break;
		default: break;
		}
		...
		if (volumeValue > 0) {
			for (int32 i = 0; i < volumeValue; i++) getText()->drawASCII(']');
			if (volumeValue == 20) getText()->drawASCII('*');
		} else if (volumeIndex) {
...
void Menu::updateSettings() {
	...
	if (Config.gammaLevel) {
		for (int32 i = 0; i < Config.gammaLevel; i++) getText()->drawASCII(']');
		if (Config.gammaLevel == 8) getText()->drawASCII('*');
	} else {
	...
	if (Config.performance == 5) {
		...
		for (int32 i = 5; i > Config.performance; --i) getText()->drawASCII(']');
		if (!Config.performance) getText()->drawASCII('*');
	}

	const char *animationsSpeed;
	switch (_vm->getLanguage()) {
	case Common::DE_DEU: animationsSpeed = "Animationen Geschwindigkeit"; break;
	case Common::ES_ESP: animationsSpeed = "Velocidad de animaciones"; break;
	case Common::FR_FRA: animationsSpeed = "Vitesse d'animations"; break;
	case Common::IT_ITA: animationsSpeed = "Velocità di animazioni"; break;
	case Common::PL_POL: animationsSpeed = "Szybkość animacji"; break;
	case Common::RU_RUS: animationsSpeed = "\x91\xaa\xae\xe0\xae\xe1\xe2\xec\x20\xa0\xad\xa8\xac\xa0\xe6\xa8\xa9"; break;
	default: animationsSpeed = "Animations speed"; break;
	}
...
void Menu::updateShowCredits() {
	...
	_startIndex -= 2;
	if (_startIndex < -(_creditsNumSteps + 24)) closeCredits();
}

void Menu::clickNewGame() {
	Common::Point cursor = LOGICAL_MOUSE(getCursor()->position());
	if (cursor.x < 247 || cursor.x > (247 + getText()->getWidth(MAKE_RESOURCE(kResourcePackText, 1322))) || cursor.y < 273 || cursor.y > (273 + 24)) {
		if (cursor.x >= 369 && cursor.x <= (369 + getText()->getWidth(MAKE_RESOURCE(kResourcePackText, 1323))) && cursor.y >= 273 && cursor.y <= (273 + 24))
			leave();
	} else {
		_vm->restart();
	}
...
void Menu::clickLoadGame() {
	...
	if (_dword_455C80) {
		if (cursor.x < 247 || cursor.x > (247 + getText()->getWidth(MAKE_RESOURCE(kResourcePackText, 1330))) || cursor.y < 273 || cursor.y > (273 + 24)) {
			if (cursor.x >= 369 && cursor.x <= (369 + getText()->getWidth(MAKE_RESOURCE(kResourcePackText, 1331))) && cursor.y >= 273 && cursor.y <= (273 + 24))
				_dword_455C80 = false;
		} else {
	...
	if (cursor.x >= 30  && cursor.x <= (30 + getText()->getWidth(MAKE_RESOURCE(kResourcePackText, 1326))) && cursor.y >= 340 && cursor.y <= (340 + 24)) {
		if (_startIndex) {
			_startIndex -= 12;
	...
	if (cursor.x >= 300 && cursor.x <= (300 + getText()->getWidth(MAKE_RESOURCE(kResourcePackText, 1328))) && cursor.y >= 340 && cursor.y <= (340 + 24)) {
		leave();
		return;
	}

	if (cursor.x >= 550 && cursor.x <= (550 + getText()->getWidth(MAKE_RESOURCE(kResourcePackText, 1327))) && cursor.y >= 340 && cursor.y <= (340 + 24)) {
		if (_startIndex + 12 < 25) {
			_startIndex += 12;
			if (_startIndex >= 25) _startIndex = 24;
		}
		return;
	}
	...
	for (int32 y = 150; y < 324; y += 29) {
		if (cursor.x >= 350) {
			...
			if (index + _startIndex > 24) break;
			snprintf(text, sizeof(text), "%d. %s", index + _startIndex + 1, getSaveLoad()->getName((uint32)(index + _startIndex)).c_str());
			if (cursor.x <= (30 + getText()->getWidth(text)) && cursor.y >= y && cursor.y <= (y + 24)) {
...
void Menu::clickSaveGame() {
	...
	if (_dword_455C80) {
		if (cursor.x < 247 || cursor.x > (247 + getText()->getWidth(MAKE_RESOURCE(kResourcePackText, 1340))) || cursor.y < 273 || cursor.y > (273 + 24)) {
			if (cursor.x >= 369 && cursor.x <= (369 + getText()->getWidth(MAKE_RESOURCE(kResourcePackText, 1341))) && cursor.y >= 273 && cursor.y <= (273 + 24))
				_dword_455C80 = false;
		} else {
			_dword_455C80 = false;
	...
	if (cursor.x >= 30  && cursor.x <= (30 + getText()->getWidth(MAKE_RESOURCE(kResourcePackText, 1336))) && cursor.y >= 340 && cursor.y <= (340 + 24)) {
		if (_startIndex) {
			_startIndex -= 12;
			if (_startIndex < 0) _startIndex = 0;
		}
	...
	if (cursor.x >= 300 && cursor.x <= (300 + getText()->getWidth(MAKE_RESOURCE(kResourcePackText, 1338))) && cursor.y >= 340 && cursor.y <= (340 + 24)) {
		leave();
		return;
	}

	if (cursor.x >= 550 && cursor.x <= (550 + getText()->getWidth(MAKE_RESOURCE(kResourcePackText, 1337))) && cursor.y >= 340 && cursor.y <= (340 + 24)) {
		if (_startIndex + 12 < 25) {
			_startIndex += 12;
			if (_startIndex >= 25) _startIndex = 24;
		}
	...
	for (int16 y = 150; y < 324; y += 29) {
		if (cursor.x >= 350) {
			...
			if (cursor.x <= (350 + getText()->getWidth(text)) && cursor.y >= y && cursor.y <= (y + 24) && getScene() && getWorld()->chapter != kChapterNone) {
			...
			if (cursor.x <= (30 + getText()->getWidth(text)) && cursor.y >= y && cursor.y <= (y + 24) && getScene() && getWorld()->chapter != kChapterNone) {
				uint32 saveIndex = (uint32)(index + _startIndex);
				if (saveIndex < 25) {
					getSaveLoad()->setIndex(saveIndex);
					if (getSaveLoad()->hasSavegame(saveIndex)) {
...
void Menu::clickDeleteGame() {
	...
	if (_dword_455C80) {
		if (cursor.x < 247 || cursor.x > (247 + getText()->getWidth(MAKE_RESOURCE(kResourcePackText, 1350))) || cursor.y < 273 || cursor.y > (273 + 24)) {
			if (cursor.x >= 369 && cursor.x <= (369 + getText()->getWidth(MAKE_RESOURCE(kResourcePackText, 1351))) && cursor.y >= 273 && cursor.y <= (273 + 24))
				_dword_455C80 = false;
		} else {
			getSaveLoad()->remove();
		}
	...
	if (cursor.x >= 30  && cursor.x <= (30 + getText()->getWidth(MAKE_RESOURCE(kResourcePackText, 1346))) && cursor.y >= 340 && cursor.y <= (340 + 24)) {
		if (_startIndex) {
			_startIndex -= 12;
			if (_startIndex < 0) _startIndex = 0;
		}
	...
	if (cursor.x >= 300 && cursor.x <= (300 + getText()->getWidth(MAKE_RESOURCE(kResourcePackText, 1348))) && cursor.y >= 340 && cursor.y <= (340 + 24)) {
		leave();
		return;
	}

	if (cursor.x >= 550 && cursor.x <= (550 + getText()->getWidth(MAKE_RESOURCE(kResourcePackText, 1347))) && cursor.y >= 340 && cursor.y <= (340 + 24)) {
		if (_startIndex + 12 < 25) {
			_startIndex += 12;
			if (_startIndex >= 25) _startIndex = 24;
		}
...
void Menu::clickViewMovies() {
	...
	if (_dword_455C78) return;

	if (cursor.x >= 30  && cursor.x <= (30 + getText()->getWidth(MAKE_RESOURCE(kResourcePackText, 1353))) && cursor.y >= 340 && cursor.y <= (340 + 24)) {
		if (_startIndex) {
			_startIndex -= 12;
			if (_startIndex < 0) _startIndex = 0;
		}
	...
	if (cursor.x >= 300 && cursor.x <= (300 + getText()->getWidth(MAKE_RESOURCE(kResourcePackText, 1355))) && cursor.y >= 340 && cursor.y <= (340 + 24)) {
		leave();
		return;
	}

	if (cursor.x >= 550 && cursor.x <= (550 + getText()->getWidth(MAKE_RESOURCE(kResourcePackText, 1354))) && cursor.y >= 340 && cursor.y <= (340 + 24)) {
		if (_startIndex + 12 < (int32)_movieCount) {
			_startIndex += 12;
			if (_startIndex >= (int32)_movieCount) _startIndex = _movieCount - 1;
		}
		return;
	}
	...
	for (int32 y = 150; y < 324; y += 29) {
		if (cursor.x >= 350) {
			if (_movieList[index + _startIndex + 6] == -1) break;
			snprintf(text, sizeof(text), "%d. %s", index + 1, getText()->get(MAKE_RESOURCE(kResourcePackText, 1359 + _movieList[index])));
			snprintf(text2, sizeof(text2), getText()->get(MAKE_RESOURCE(kResourcePackText, 1356)), moviesCd[_movieList[index]]);
			Common::strcat_s(text, text2);
			...
			if (_movieList[index + _startIndex] == -1) break;
			snprintf(text, sizeof(text), "%d. %s", index + 1, getText()->get(MAKE_RESOURCE(kResourcePackText, 1359 + _movieList[index])));
			snprintf(text2, sizeof(text2), getText()->get(MAKE_RESOURCE(kResourcePackText, 1356)), moviesCd[_movieList[index]]);
			Common::strcat_s(text, text2);
...
void Menu::clickQuitGame() {
	Common::Point cursor = LOGICAL_MOUSE(getCursor()->position());
	if (cursor.x < 247 || cursor.x > (247 + getText()->getWidth(MAKE_RESOURCE(kResourcePackText, 1409))) || cursor.y < 273 || cursor.y > (273 + 24)) {
		if (cursor.x >= 369 && cursor.x <= (369 + getText()->getWidth(MAKE_RESOURCE(kResourcePackText, 1410))) && cursor.y >= 273 && cursor.y <= (273 + 24)) {
			leave();
		}
	} else {
		getCursor()->hide();
