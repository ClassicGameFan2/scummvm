...
void Screen::draw(GraphicResource *resource, uint32 frameIndex, const Common::Point &source, DrawFlags flags, ResourceId resourceIdDestination, const Common::Point &destination, bool colorKey) {
	...
	Common::Rect src, dest, srcMask, destMask;

	dest.left = source.x + frame->x;
	if (flags & kDrawFlagMirrorLeftRight) {
		if (_flag == -1) {
			if ((resource->getData().flags & 15) >= 2) dest.left = source.x + (int16)resource->getData().maxWidth - ((int16)frame->getWidth() + frame->x);
		} else {
	...
	src.left = 0; src.top = 0; src.right = frame->getWidth(); src.bottom = frame->getHeight();
	...
	if (resourceIdDestination) {
		...
		if (!dest.intersects(destMask)) masked = false;
	}
	...
	if (S > 1) {
		...
		if (hdSurf) {
			...
			if (masked) {
				...
				if (flags & kDrawFlagMirrorLeftRight) {
					mirroredBuffer = (byte *)malloc((size_t)(hdSrc.right * hdSrc.bottom));
					blitMirrored(mirroredBuffer, frameBuffer + hdSrc.right - 1, hdSrc.bottom, hdSrc.right, hdSrc.right + frameRight, 0);
					frameBuffer = mirroredBuffer;
					frameRight = hdSrc.right;
					hdSrc.right -= hdSrc.left;
					hdSrc.left = 0;
				}
		...
		for (int cy = 0; cy < surface.h; cy++) {
			...
			for (int cx = 0; cx < surface.w; cx++) {
				if (srcRow[cx]) {
					for (int sy = 0; sy < S; sy++) {
						for (int sx = 0; sx < S; sx++) {
							hdPixels[((y+cy)*S + sy) * _hdBackBuffer.pitch + ((x+cx)*S + sx)] = srcRow[cx];
						}
					}
				}
			}
...
void Screen::copyToBackBufferWithTransparency(byte *buffer, int32 pitch, int16 x, int16 y, uint16 width, uint16 height, bool mirrored) {
	...
	// DYNAMIC LIMITS: Because Screen::draw swaps _backBuffer to _hdBackBuffer, 
	// this dynamically adapts to 1708 bounds! Max's pixels will never mangle or clip!
	int32 bw = _backBuffer.w;
	int32 bh = _backBuffer.h;
	int32 bp = _backBuffer.pitch;

	int32 right = (x + width > bw) ? bw - abs(x) : width;
	int32 bottom = (y + height > bh) ? bh - abs(y) : height;
	...
	for (int32 curY = top; curY < bottom; curY++) {
		for (int32 curX = left; curX < right; curX++) {
			...
			if (buffer[offset] != 0) dest[x + curX + (y + curY) * bp] = buffer[offset];
		}
	}
	
	// DIRTY RECT UPSCALE: Safely scale 1x UI components to the HD Buffer.
	int scale = ASYLUM_SCALE_FACTOR;
	if (scale > 1 && bw == LOGICAL_WIDTH) {
		byte *hdDest = (byte *)_hdBackBuffer.getPixels();
		int32 hdPitch = _hdBackBuffer.pitch;
		for (int32 curY = top; curY < bottom; curY++) {
			for (int32 curX = left; curX < right; curX++) {
...
void Screen::loadGrayPalette() {
	...
	if (!paletteId) paletteId = getWorld()->currentPaletteId;
...
void Screen::setupPalette(byte *buffer, int start, int count) {
	if (start < 0 || start > 256) error("[Screen::setupPalette] Invalid start parameter");
	if ((count + start) > 256) error("[Screen::setupPalette] Parameters go past the palette buffer");
...
void Screen::queuePaletteFade(ResourceId resourceId, int32 ticksWait, int32 delta) {
	if (_isFading && !_fadeQueue.empty() && _fadeQueue.front().resourceId == resourceId) return;
	if (ticksWait < 0 || delta <= 0) return;
...
void Screen::paletteFade(uint32 start, int32 ticksWait, int32 delta) {
	if (start > 255 || ticksWait < 0 || delta <= 0) return;
	...
	for (int32 i = 1; i < colorDelta; i++) {
		...
		g_system->delayMillis((uint32)ticksWait);
		Common::Event ev;
		do { } while (_vm->getEventManager()->pollEvent(ev));
		g_system->updateScreen();
	}
}

void Screen::processPaletteFadeQueue() {
	if (_fadeQueue.empty()) return;
	...
	if (_vm->getTick() > current->nextTick) {
		if (current->step > current->delta) {
			...
			if (current->step == 1) initQueuedPaletteFade(current->resourceId, current->delta);
...
void Screen::setPaletteGamma(byte *data, byte *target) {
	if (target == nullptr) target = (byte *)&_mainPalette;
	data += 4;
...
void Screen::setupTransTables(uint32 count, ...) {
	if (!count) { clearTransTables(); return; }
	va_list va;
	va_start(va, count);
...
void Screen::addGraphicToQueueCrossfade(ResourceId resourceId, uint32 frameIndex, const Common::Point &point, int32 objectResourceId, const Common::Point &destination, uint32 transTableNum) {
	...
	if (src.isValidRect()) {
		...
		blitCrossfade((byte *)_backBuffer.getPixels() + dst.top * _backBuffer.pitch + dst.left,
		              ...
		              dst.height(), dst.width(), (uint16)(frame->surface.pitch - dst.width()),
		              (uint16)(_backBuffer.pitch - dst.width()), (uint16)(frameObject->surface.pitch - dst.width()));
		...
		if (S > 1) {
			...
			if (hdF && hdO) {
				...
				blitCrossfade((byte *)_hdBackBuffer.getPixels() + s_dst.top * _hdBackBuffer.pitch + s_dst.left,
				              (byte *)hdF->getPixels() + s_src.top * hdF->pitch + s_src.left,
				              (byte *)hdO->getPixels() + (destination.y * S + s_dst.top) * hdO->pitch + (s_dst.left + destination.x * S),
				              s_dst.height(), s_dst.width(), (uint16)(hdF->pitch - s_dst.width()),
				              (uint16)(_hdBackBuffer.pitch - s_dst.width()), (uint16)(hdO->pitch - s_dst.width()));
			}
		}
	}
...
void Screen::copyToBackBufferClipped(Graphics::Surface *surface, int16 x, int16 y) {
	...
	if (!animRect.isEmpty()) {
		...
		if (surface->w > LOGICAL_WIDTH) startX = getWorld()->xLeft;
		if (surface->h > LOGICAL_HEIGHT) startY = getWorld()->yTop;
