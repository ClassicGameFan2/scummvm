...
void Screen::copyBackBufferToScreen() {
	int S = ASYLUM_SCALE_FACTOR;
	if (S > 1) {
		// ULTRA-FAST FAILSAFE SYNC: Restores missing Chapter Titles and prevents Max flickering without lagging the CPU!
		...
		int pitch1 = _backBuffer.pitch;
		int pitch2 = _hdBackBuffer.pitch;

		for (int y = 0; y < h; y++) {
			const byte *srcRow = src + (y * pitch1);
			for (int x = 0; x < w; x++) {
				byte c = srcRow[x];
				if (c > 0 && c != bgCol) {
					int dx = x * S;
